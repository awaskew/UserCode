...
HybridClusterAlgo::HybridClusterAlgo(double eb_str, 
				     ...
				     bool robustSeeding,
				     bool robustClustering,
				     double robustTimeCons
				     ) :
  
  eb_st(eb_str), //Seed threshold
  phiSteps_(step), //Number of steps in phi
  eThres_(ethres), 
  eThresA_(eThresA),  
  eThresB_(eThresB),
  robustSeeding_(robustSeeding),
  robustClustering_(robustClustering),
  robustTimeCons_(robustTimeCons), 
  Eseed(eseed),  
  Ewing(ewing), 
  ...
  
{
  ...
  std::cout << "Currently have robust seeding set to: " << robustSeeding_ << std::endl;
  if (robustSeeding_) std::cout << "Alternative procedure will be used for avoiding spikes in clusters." << std::endl;
}
				     ...
				     )
{
  ...
  //This bit is for the regional reconstruction for HLT
  if(regional) nregions=regions.size();

  //Of course, if you're not doing that, or if you actually have a
  ...
  if(!regional || nregions) {
	...
	//Look, it's the seed threshold parameter!
	if (ET > eb_st) {
	  ...
	  if (!robustSeeding_){
	    ...
	    //Robust seeding.  Look in adjacent crystals in 3x3
	    //find those with requisite energy (at least
	    //10% of seed energy.  Then check time consistancy
	    
	    //Make a navigator, and set it to the seed cell candidate.
	    EcalBarrelNavigator cursor(it->id(), topo_);
	    std::vector<EcalRecHit> CrysArray;
	    std::vector<EcalRecHit> EnergyRat;
	    std::vector<EcalRecHit> TimeCons;
	    Float_t SeedEnergy = it->energy();
	    Float_t SeedTime = it->time();
	    Float_t SeedTimeUncert = it->timeError();
	    for ( int eastNr = -1; eastNr <= 1; ++eastNr ) { //east is eta in barrel
	      for ( int northNr = -1; northNr <= 1; ++northNr ) { //north is phi in barrel
		if (eastNr==0 && northNr==0) continue;
		cursor.home();
		cursor.offsetBy( eastNr, northNr);
		EcalRecHitCollection::const_iterator neigh = recHits_->find(*cursor);
		if (neigh!=recHits_->end()){
		  CrysArray.push_back(*neigh);
		  Float_t CrysE = neigh->energy();
		  Float_t RatE = CrysE/SeedEnergy;
		  if (RatE > 0.03 && CrysE>eb_st){
		    EnergyRat.push_back(*neigh);
		    Float_t CrysT = neigh->time();
		    Float_t CrysTErr = neigh->timeError();
		    Float_t DiffDivErr = fabs(CrysT - SeedTime)/sqrt(CrysTErr*CrysTErr + SeedTimeUncert*SeedTimeUncert);
		    if (DiffDivErr<robustTimeCons_)
		      TimeCons.push_back(*neigh);
		  }
		}
	      } //end east loop
	    }//end north loop
...
void HybridClusterAlgo::mainSearch(const EcalRecHitCollection* hits, const CaloSubdetectorGeometry*geometry)
{
	...
	for (it = seeds.begin(); it != seeds.end(); it++){
	  ...
	  float SeedTime = it->time();
	  float SeedTErr = it->timeError();
	  double e_init = makeDomino(navigator, initialdomino, SeedTime, SeedTErr);
	  ...
	  for (int i=0;i<phiSteps;++i){
	    ...
	    double etemp = makeDomino(dominoNav, dcells, SeedTime, SeedTErr);
...
double HybridClusterAlgo::makeDomino(EcalBarrelNavigator &navigator, std::vector <EcalRecHit> &cells, float SeedCrysTime, float SeedCrysTErr)
{
	...
	if (center_it!=recHits_->end()){
	  ...
	  if (useddetids.find(center) == useddetids.end() && excludedCrys_.find(center)==excludedCrys_.end()){ 
	    if (robustClustering_){
	      float crysTime = SeedHit.time();
	      float crysE = SeedHit.energy();
	      float crysTErr = SeedHit.timeError();
	      float TDiffDiv = fabs(crysTime-SeedCrysTime)/sqrt(crysTErr*crysTErr + SeedCrysTErr*SeedCrysTErr);
	      if ( (crysE>eb_st && TDiffDiv < robustTimeCons_) || (crysE<eb_st)){
		Etot += SeedHit.energy();
	...
	if (eta1_it !=recHits_->end()){
	  ...
	  if (useddetids.find(ieta1) == useddetids.end() && excludedCrys_.find(ieta1)==excludedCrys_.end()){
	    if (robustClustering_){
	      float crysTime = UpEta.time();
	      float crysE = UpEta.energy();
	      float crysTErr = UpEta.timeError();
	      float TDiffDiv = fabs(crysTime-SeedCrysTime)/sqrt(crysTErr*crysTErr + SeedCrysTErr*SeedCrysTErr);
	      if ( (crysE>eb_st && TDiffDiv<robustTimeCons_) || (crysE<eb_st)){
		Etot += UpEta.energy();
	...
	if (eta2_it !=recHits_->end()){
	  ...
	  if (useddetids.find(ieta2)==useddetids.end() && excludedCrys_.find(ieta2)==excludedCrys_.end()){
	    if (robustClustering_){
	      float crysTime = DownEta.time();
	      float crysE = DownEta.energy();
	      float crysTErr = DownEta.timeError();
	      float TDiffDiv = fabs(crysTime-SeedCrysTime)/sqrt(crysTErr*crysTErr + SeedCrysTErr*SeedCrysTErr);
	      if ( (crysE>eb_st && TDiffDiv < robustTimeCons_) || (crysE<eb_st)){
		Etot += DownEta.energy();
	...
	if (ieta2 != DetId(0)){
	  ...
	  if (eta3_it != recHits_->end()){
	    ...
	    if (useddetids.find(ieta3)==useddetids.end() && excludedCrys_.find(ieta3)==excludedCrys_.end()){
	      if (robustClustering_){
		float crysTime = DownEta2.time();
		float crysE = DownEta2.energy();
		float crysTErr = DownEta2.timeError();
		float TDiffDiv = fabs(crysTime-SeedCrysTime)/sqrt(crysTErr*crysTErr + SeedCrysTErr*SeedCrysTErr);
		if ( (crysE>eb_st && TDiffDiv<robustTimeCons_) || (crysE<eb_st)){
	...
	if (ieta1 != DetId(0)){
	  ...
	  if (eta4_it != recHits_->end()){
	    ...
	    if (useddetids.find(ieta4) == useddetids.end() && excludedCrys_.find(ieta4)==excludedCrys_.end()){
	      if (robustClustering_){
		float crysTime = UpEta2.time();
		float crysE = UpEta2.energy();
		float crysTErr = UpEta2.timeError();
		float TDiffDiv = fabs(crysTime-SeedCrysTime)/sqrt(crysTErr*crysTErr + SeedCrysTErr*SeedCrysTErr);
		if ( (crysE>eb_st && TDiffDiv < robustTimeCons_) || (crysE<eb_st)){
		  Etot += UpEta2.energy();
